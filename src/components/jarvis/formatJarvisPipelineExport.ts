...
function formatAgentStep(step: JarvisExecutionStep): string {
  ...
  if (step.agentId === 'coder') {
    const lines: string[] = [`=== ${agentTitle} ===`];
    lines.push(raw || step.outputPreview || step.summary || 'Code implementation completed.');
    return lines.join('\n').trim();
  }
...
export function formatFullPipelineExport(
  ...
  // Check if this is the dedicated 2-agent /code pipeline (where finalSynthesizer was skipped)
  const isCodePipelineOnly =
    steps.some((s) => s.agentId === 'coder' && s.status === 'completed') &&
    steps.some((s) => s.agentId === 'finalSynthesizer' && s.status === 'skipped');

  const isAutoCodePipeline =
    steps.some((s) => s.agentId === 'coder' && s.status === 'completed') &&
    steps.some((s) => s.agentId === 'finalSynthesizer' && s.status === 'completed');

  if (isCodePipelineOnly) {
    // 1. "/code" SLASH COMMAND — Report shows only Planner and Coder sections.
    const codeSteps = steps.filter(
      (s) => (s.agentId === 'planner' || s.agentId === 'coder') && s.status === 'completed',
    );
    codeSteps.forEach((step, idx) => {
    ...
    // 2. AUTOMATICALLY DETECTED CODING PIPELINE — 4 sections: Planner, Coder, Reviewer, Final Synthesizer
    const targetOrder = ['planner', 'coder', 'reviewer'];
    const autoCodeSteps = targetOrder
      .map((id) => steps.find((s) => s.agentId === id && s.status === 'completed'))
      .filter((s): s is JarvisExecutionStep => Boolean(s));

    autoCodeSteps.forEach((step) => {
      const formatted = formatAgentStep(step);
      if (formatted) {
        exportParts.push(formatted);
        exportParts.push(`\n--------------------------------------------------------------------------------\n`);
    ...
    // 4th section: Final Synthesizer
    exportParts.push(`=== FINAL SYNTHESIZER ===\n`);
