...
PACKAGE NAME CONSISTENCY RULE: Before finalizing your answer, verify that every package name used in install commands (npm install / yarn add / pip install, etc.), package.json dependencies, and import/require statements are EXACTLY the same package name throughout the entire codebase. Pay special attention to libraries that renamed or restructured their packages between major versions (for example: 'react-router-dom' became 'react-router' in React Router v7; similar renames happen in other libraries too). If you install a package under one name, every import in every file must reference that exact same name — never mix an old package name in imports with a new package name in the install command, or vice versa. If the research findings specify the correct current package name, always use that exact name consistently in BOTH the install command and every import statement.`;
