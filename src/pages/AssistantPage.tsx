...
export function AssistantPage() {
  ...
  const handleUniversalCopyAll = useCallback(async () => {
    ...
    if (success) {
      ...
      triggerSettingsToast('Universal Copy: Copied all AI Assistant answers & conversation text!');
    }
  }, [messages]);
  ...
  const handleUniversalSaveAll = useCallback(() => {
    ...
    // 1. Save all individual answers properly categorized (diagram, chart, or jarvis)
    ...
    // 2. Also save full conversation session archive
    ...
    triggerSettingsToast(`Universal Save: Stored all ${savedCount} answers to your Library in Saved Page!`);
  }, [messages, savedItemIds]);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap justify-end">
          {/* Universal Action Buttons in Header when conversation exists */}
          {!isOnlyWelcome && messages.some((m) => m.role === 'assistant' && !m.isWelcome) && (
            <div className="flex items-center gap-1 bg-zinc-900/90 border border-zinc-700/70 p-1 rounded-xl shadow-sm mr-1">
              {/* Universal Copy All */}
              <button
                type="button"
                onClick={handleUniversalCopyAll}
                className={`text-xs px-2 py-1 rounded-lg transition-all flex items-center gap-1.5 ${
                  universalCopied
                    ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'
                    : theme === 'classic'
                    ? 'text-cyan-200 hover:bg-cyan-950/50'
                    : theme === 'fulldark'
                    ? 'text-[#d4d4d8] hover:bg-[#282828] hover:text-white'
                    : 'text-zinc-300 hover:bg-zinc-800 hover:text-white'
                }`}
                title="Universal Copy: Copy all AI Assistant answers and conversation text"
              >
                {universalCopied ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />}
                <span className="hidden md:inline font-medium">
                  {universalCopied ? 'Copied All' : 'Copy All'}
                </span>
              </button>

              {/* Universal Edge TTS Speaker All */}
              <button
                type="button"
                ...
                title={
                  universalEdgeTtsLoading
                    ? 'Synthesizing Edge TTS for all answers...'
                    : universalEdgeTtsPlaying
                    ? 'Stop listening to all answers'
                    : 'Universal Edge TTS: Listen to all AI Assistant answers'
                }
              >
                {universalEdgeTtsLoading ? (
                ...
                <span className="hidden md:inline font-medium">
                  {universalEdgeTtsPlaying ? 'Stop TTS' : 'Listen All'}
                </span>
              </button>
              ...
              <button
                ...
                title="Universal Save: Save all AI Assistant answers to your Library in Saved Page"
              >
                <Bookmark
                  size={13}
                ...
                />
                <span className="hidden md:inline font-medium">
                  {universalSaved ? 'Saved All' : 'Save All'}
                </span>
              </button>
            </div>
          )}
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto flex flex-col justify-between py-6 px-1 sm:px-2">
        <div className="flex-1 space-y-6">
          ...
          {/* Universal Conversation Controls Banner (All AI Assistant Answers & Text) */}
          {!isOnlyWelcome && messages.some((m) => m.role === 'assistant' && !m.isWelcome) && (
            <div
              className={`px-3.5 py-2.5 rounded-2xl border flex items-center justify-between gap-3 flex-wrap transition-all shadow-sm ${
                theme === 'classic'
                  ? 'bg-slate-900/80 border-cyan-500/30 text-cyan-200'
                  : theme === 'fulldark'
                  ? 'bg-[#181818] border-[#2a2a2a] text-[#cfcfcf]'
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="flex flex-col">
                  <span className="text-xs font-semibold text-zinc-100 flex items-center gap-1.5">
                    Universal Conversation Actions
                    <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-cyan-500/15 text-cyan-300 font-medium">
                      All Answers &amp; Text
                    </span>
                  </span>
                  <span className="text-[11px] text-zinc-400">
                    Copy, speak via Edge TTS, or save all assistant answers to your Saved Page
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 shrink-0 flex-wrap">
                ...
                <button
                  ...
                  className={`text-xs px-2.5 py-1.5 rounded-xl border transition-all flex items-center gap-1.5 font-medium ${
                    universalCopied
                      ...
                      : theme === 'classic'
                      ? 'border-cyan-500/30 bg-cyan-950/50 text-cyan-200 hover:bg-cyan-900/60 hover:text-white'
                      : theme === 'fulldark'
                      ? 'border-[#333] bg-[#222] text-[#ccc] hover:bg-[#2c2c2c] hover:text-white'
                      : 'border-zinc-700/60 bg-zinc-800/80 text-zinc-200 hover:bg-zinc-700 hover:text-white'
                  }`}
                  title="Universal Copy: Copy entire conversation transcript & all answers"
                >
                  {universalCopied ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />}
                  <span>{universalCopied ? 'Copied All Answers' : 'Universal Copy'}</span>
                </button>

                {/* Universal Edge TTS Speaker */}
                <button
                  type="button"
                  onClick={handleUniversalEdgeTtsSpeakAll}
                  disabled={universalEdgeTtsLoading}
                  ...
                  title={
                    universalEdgeTtsLoading
                      ...
                      : 'Universal Edge TTS: Listen to all AI Assistant answers read aloud'
                  }
                >
                  {universalEdgeTtsLoading ? (
                    <Loader2 size={13} className="animate-spin text-purple-400" />
                ...
                <button
                  ...
                  title="Universal Save: Save all AI Assistant answers into your Saved Page category"
                >
                  <Bookmark
                    size={13}
                    className={universalSaved ? 'fill-amber-400 text-amber-400' : ''}
                  />
                  <span>{universalSaved ? 'All Answers Saved!' : 'Universal Save'}</span>
                </button>
              </div>
            </div>
          )}
