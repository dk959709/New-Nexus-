...
/**
 * Validates whether the fetched content is actually a "fake success" error page
 * (e.g. CloudFront 403 error page, Cloudflare challenge, or bot-protection block page
 * returned with an HTTP 200 OK status code).
 */
function isBlockedOrErrorPage(rawHtml: string, textContent: string, title: string = ''): boolean {
  const combined = `${title} ${textContent} ${rawHtml.slice(0, 5000)}`.toLowerCase();

  // 1. Specific CloudFront 403 / block signatures
  if (
  ...
  // 2. Cloudflare challenge / bot protection signatures
  if (
  ...
  // 3. Generic 403 / Access Denied / 405 error pages masquerading as valid responses
  if (
  ...
  // 4. Bot challenge / CAPTCHA pages with very little actual content
  if (
...
/**
 * Executes the ordered fallback chain:
 * Fallback 1: Headless Chromium (Playwright)
 * Fallback 2: Browserless / External Proxy (RENDER_PROXY_URL)
 * Fallback 3: Jina Reader (https://r.jina.ai/<url>)
 */
async function executeFallbackCascade(
...
async function fetchDirectWebPage(targetUrl: string): Promise<{
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Direct fetch was blocked or failed (e.g. 403 Forbidden, 405 Method Not Allowed, 429 Rate Limit, 500/503).
      // Trigger fallback cascade (Headless -> Browserless -> Jina Reader)
    ...
    // Check for "Fake Success": CloudFront 403 error page, Cloudflare challenge, or WAF block served with status 200
    ...
    // Check for JS-gated pages (SPAs, noscript warnings, empty div#root)
    ...
    // Direct fetch succeeded with genuine, readable content
    return {
      ok: true,
      data: {
        url: normalizedUrl,
