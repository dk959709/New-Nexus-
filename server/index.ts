...
async function startServer() {
  ...
  app.post('/api/ai/chat', async (req, res) => {
    ...
    const memoryContext = parsed.data.memory?.trim() ? parsed.data.memory.slice(-1200) : '';
    const systemPrompt = [
    ...
    // Try Gemini first if GEMINI_API_KEY is configured
    ...
    if (!key || !url) {
      return errorResponse(
        res,
        503,
        'AI Assistant is not configured. Add GEMINI_API_KEY or AI_API_KEY and AI_API_URL to the server environment.',
      );
    }
