...
async function geocode(city) {
    ...
    if (!response.ok)
        throw new Error('Weather provider is temporarily unavailable.');
    const payload = await response.json();
...
async function weatherProvider(latitude, longitude, location) {
...
app.post('/api/ai/chat', async (req, res) => {
    ...
    const messages = [
        {
            role: 'system',
            content: 'You are NEXUS AI, a helpful, concise assistant inside the NEXUS Intelligence app. Give clear and useful answers. Do not claim to have live information unless it is provided by NEXUS.',
        },
        ...(parsed.data.history ?? []),
        {
            role: 'user',
    ...
    try {
        ...
        if (!upstream.ok) {
            return errorResponse(res, 502, payload.error?.message ?? 'AI provider is temporarily unavailable.');
        }
...
app.post('/api/search/summary', async (req, res) => { const parsed = z.object({ query: z.string().min(1), results: z.array(z.object({ title: z.string(), url: z.string(), description: z.string() })).max(20) }).safeParse(req.body); if (!parsed.success)
    return errorResponse(res, 400, 'A query and search results are required.'); const key = process.env.AI_API_KEY; const url = process.env.AI_API_URL; if (!key || !url)
    return errorResponse(res, 503, 'AI summary is not configured.'); try {
    const response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` }, body: JSON.stringify({ model: process.env.AI_MODEL, messages: [{ role: 'user', content: `Answer only from these sources. Query: ${parsed.data.query}\nSources: ${JSON.stringify(parsed.data.results)}` }] }) });
    if (!response.ok)
        throw new Error();
...
app.get('/api/weather/geocode', async (req, res) => { const parsed = z.string().trim().min(1).max(120).safeParse(req.query.city); if (!parsed.success)
    return errorResponse(res, 400, 'Enter a city.'); try {
    return res.json({ data: await geocode(parsed.data) });
}
catch {
    return errorResponse(res, 502, 'Weather provider is temporarily unavailable.');
} });
app.get('/api/weather', async (req, res) => { const parsed = weatherSchema.safeParse(req.query); if (!parsed.success)
    return errorResponse(res, 400, 'Provide a city or coordinates.'); try {
    ...
    return errorResponse(res, 502, error instanceof Error ? error.message : 'Weather provider is temporarily unavailable.');
} });
...
app.get('/api/news', async (_req, res) => { try {
    return res.json({ data: await searchProvider({ query: 'latest world news', category: 'NEWS' }) });
}
catch (error) {
    const err = error;
    return errorResponse(res, err.status ?? 502, err.message);
